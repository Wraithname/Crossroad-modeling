...
namespace PracticaSummer
{
    ...
    public class Cars:BaseClass
    {
        ...
        public Cars(int x, int y, int speed, Bitmap sprite, int nspr, Vector dir, int polosa, int src, int frc, Side cr, CTurn ct)
        {
            X = x;
            Y = y;
            Speed = speed;
        ...
        public static int CanMove(Cars c)
        {
            switch (c.StartRoad)
            {
                case Side.Down:
        ...
        public static bool Check(Cars c)
        {
            switch (c.CurrRoad)
            {
                case Side.Down:
                    if (!RoadPass.CenterFive(c) && Engine.Car.Exists(
                        sc =>
                            sc.Y == c.Y &&
                            c.X + 60 >= sc.X && c.X + 10 <= sc.X))
                        return true;
                    if (c.Turn == CTurn.Left && Engine.TrafficLights[2].CurrLight == TrafficLight.Lights.Green && c.X <= Engine.UserPanel.Width / 2 - 60 && c.X >= Engine.UserPanel.Width / 2 - 65)
                        return true;
                ...
                case Side.Left:
                    if (!RoadPass.CenterFive(c) && Engine.Car.Exists(
                        sc =>
                            ...
                            c.Y + 60 >= sc.Y && c.Y + 10 <= sc.Y))
                        return true;
                    if (c.Turn == CTurn.Left && Engine.TrafficLights[0].CurrLight == TrafficLight.Lights.Green && c.Y <= Engine.UserPanel.Height / 2 - 55 && c.Y >= Engine.UserPanel.Height / 2 - 60)
                        return true;
